// GLOBAL API BASE CONFIGURATION (REPLACE WITH YOUR CLOUDFLARE OR NGROK TUNNEL URL WHEN LIVE!)
...
async function sendTestNotification(channel) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/send-test-notification", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
async function refreshActivePositions() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/open-trades");
        const data = await response.json();
...
function openSymbolWindow(symbol) {
    window.open(`./symbol_detail.html?symbol=${symbol}`, `_blank`, `width=1200,height=800,toolbar=no,menubar=no,scrollbars=yes,resizable=yes`);
}
